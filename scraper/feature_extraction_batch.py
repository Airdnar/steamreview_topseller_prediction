...
NUM_TOPICS = 4  # Number of topics for each LDA
TOP_N = 30      # Top-N words per topic for inspection

def main():
    # Step 1: Fetch top-selling games (first 4 pages), keep top 5
    ...
    top_200 = top_sellers.copy()

    os.makedirs('data', exist_ok=True)
    top_200.to_csv('data/top_200_sellers.csv', index=False)
    print("Fetched top 200 sellers:\n", top_200[['appid','title']])

    # Step 2: Download reviews per language for each game
    os.makedirs('data/reviews', exist_ok=True)
    for _, row in top_200.iterrows():
        appid, title, rank = row['appid'], row['title'],  row['rank']
    ...
    for lang in languages:
        ...
        json_path = f"data/features/{lang}_topic_terms.json"
    ...
    for _, row in top_200.iterrows():
        ...
        record = {'appid': appid, 'title': title, 'rank': rank}
        for lang in languages:
    ...
    cols = ['rank', 'appid', 'title'] + [c for c in combined.columns if c not in ('rank','appid','title')]
    combined = combined[cols]  # reorder columns

    combined.to_csv('data/features/all_games_multilang_features.csv', index=False)
    print("Saved multilingual feature matrix to data/features/all_games_multilang_features.csv")
